...
const TalkToExpertForm = ({ variant = "landing" }) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setSubmitStatus({
          type: "success",
          message:
            "Message sent successfully! Our expert will contact you soon.",
        });
        setFormData({
          name: "",
          email: "",
  ...
  const selectClass = isContactPage
    ...
    : "w-full px-3 sm:px-4 py-2 sm:py-3 border-b-2 border-gray-200 focus:border-[#4555A7] outline-none transition-colors bg-transparent text-gray-700 appearance-none cursor-pointer text-sm sm:text-base hover:border-[#4555A7]";
      ...
      {isContactPage && (
        <div>
          ...
          <p className="text-sm text-gray-600 mb-6">
            Connect with our specialists to discuss your specific needs and
            requirements.
          </p>
        </div>
      )}
        ...
        {!isContactPage && (
          <div className="flex gap-1 sm:gap-2">
            <select
              ...
              onChange={handleChange}
              disabled={isSubmitting}
              className="px-1 sm:px-2 py-2 sm:py-3 border-b-2 border-gray-200 focus:border-[#4555A7] outline-none transition-colors bg-white w-24 sm:w-32 text-xs sm:text-sm disabled:opacity-50 cursor-pointer text-gray-700 hover:border-[#4555A7]"
              style={{
                borderRadius: "4px",
              }}
            >
            ...
            <input
              ...
              required
              disabled={isSubmitting}
              className="flex-1 px-3 sm:px-4 py-2 sm:py-3 border-b-2 border-gray-200 focus:border-[#4555A7] outline-none transition-colors bg-transparent text-sm sm:text-base disabled:opacity-50"
            />
          </div>
        )}
        ...
        <div>
          {isContactPage ? (
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-auto min-w-[181.5px] h-[51.6px] rounded-[5px] bg-white border border-[#3E234C] shadow-sm flex items-center justify-center gap-2 group px-6 disabled:opacity-50 disabled:cursor-not-allowed"
